...
}) => api<{ message: string; user: User }>("/register/institution", { body: payload })
...
export const login = (email: string, password: string) =>
  api<LoginResponse>("/login", { body: { email, password } })

/* =========================================================
   AUTHENTICATED (auth:sanctum)
   ========================================================= */

export const logout = () => api<{ message: string }>("/logout", { method: "POST" })

export const pendingApproval = () => api<PendingApproval>("/pending-approval")

export const verificationNotice = () => api<unknown>("/email/verification-notification")
...
export const notifications = () => api<Paginated<AppNotification>>("/notifications")
...
export const uploadDocument = (type: InstitutionDocument["type"], file: File) => {
...
export const uploadConsultantDocument = (
  ...
  return api<ConsultantDocument>(`/consultants/${consultantId}/documents`, { form })
}
...
/* Training officers */
export const listTrainingOfficers = () => api<import("./types").TrainingOfficer[]>("/training-officers")
...
) => api<ResidentTransfer>(`/residents/${residentId}/transfers`, { body: payload })

export const incomingTransfers = () => api<ResidentTransfer[]>("/transfers/incoming")
...
export const assignRotation = (rotationId: number, resident_id: number) =>
  api<RotationAssignment>(`/rotations/${rotationId}/assignments`, { body: { resident_id } })
